...
const Announcements: React.FC<{ match: any }> = ({ match }) => {
  ...
  const loadAllAnnouncements = () => {
    axios
      .get(`/classrooms/getAllAnnouncements/${classroomID}`)
      .then((response: any) => {
    ...
    <IonPage>
      <IonContent>
        <div className="ion-text-center">
          <h1>Announcements</h1>
          <Link to="/page/classrooms" className="noUnderline">
            Go back to classrooms
          </Link>
          {reversedAnnouncementList &&
          reversedAnnouncementList.map &&
          reversedAnnouncementList.length ? (
